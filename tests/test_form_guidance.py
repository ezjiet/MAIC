...
class RecommendationTests(unittest.TestCase):
    def test_recommendation_uses_retrieved_form_metadata(self):
        chunks = [{"source": "KWSP_9F_AHL_Borang_Pengeluaran.pdf", "agency": "KWSP", "text": "withdrawal application form", "page": 1}]
    ...
    def test_non_form_source_is_never_recommended(self):
        chunks = [{"source": "Member_Guide.pdf", "agency": "KWSP", "text": "withdrawal application", "page": 1}]
    ...
    def test_less_relevant_retrieved_forms_are_filtered(self):
        chunks = [
            {"source": "KWSP_9FA_AHL_Borang_Pengeluaran_Akaun_Fleksibel.pdf", "agency": "KWSP", "text": "flexible account withdrawal", "page": 1},
            {"source": "KWSP_Form_Pindah_Akaun_2_Ke_Akaun_1.pdf", "agency": "KWSP", "text": "account withdrawal options including fleksibel", "page": 1},
        ]
    ...
    def test_renewal_query_does_not_recommend_non_renewal_licence_form(self):
        chunks = [{
            "source": "JPJ_Borang_Permohonan_Lesen_Memandu_JPJ_L1.pdf",
            "agency": "JPJ",
    ...
    def test_letter_prefixed_kwsp_code_is_preserved(self):
        metadata = normalize_form_metadata({
            "source": "KWSP_Form_KWSP_R1_AHL.pdf",
            "agency": "KWSP",
...
class RetrievalGroundingTests(unittest.TestCase):
    ...
    def test_exact_form_retrieval_cannot_be_displaced_by_another_form(self):
        ...
        self.assertEqual([chunk["source"] for chunk in selected], [
            "KWSP_Form_KWSP_R1_AHL.pdf"
        ])

    def test_bare_agency_definition_does_not_cite_transaction_forms(self):
        chunks = [{
            "source": "KWSP_Form_KWSP_15_Transf_Saving_from_Monetary_Groups.pdf",
            "agency": "KWSP",
            "text": "transfer form",
            "page": 1,
        }]
    ...
    def test_explicit_form_request_keeps_only_equally_strong_secondary_sources(self):
        chunks = [
            {"source": "JPJ_Borang_Permohonan_Lesen_Memandu_JPJ_L1.pdf", "_retrieval_score": 0.82},
            {"source": "JPJ_Borang_Permohonan_Lesen_Memandu_JPJ_L1.pdf", "_retrieval_score": 0.56},
            {"source": "JPJ_Borang_Permohonan_Pengecualian.pdf", "_retrieval_score": 0.73},
        ]
        selected = filter_context_for_intent("Which form do I need for a driving licence?", "JPJ", chunks)
        self.assertEqual({chunk["source"] for chunk in selected}, {
            "JPJ_Borang_Permohonan_Lesen_Memandu_JPJ_L1.pdf"
        })
    ...
    def test_malay_house_form_request_rejects_nearby_registration_form(self):
        chunks = [
            {"source": "KWSP_9C_AHL_D5.pdf", "_retrieval_score": 0.983},
            {"source": "KWSP_9C_AHL_D5.pdf", "_retrieval_score": 0.961},
            {"source": "KWSP_Form_KWSP_3_Daftar.pdf", "_retrieval_score": 0.934},
        ]
        selected = filter_context_for_intent(
            "Saya nak guna KWSP untuk beli rumah. Borang apa saya perlu?", "KWSP", chunks
        )
        self.assertEqual({chunk["source"] for chunk in selected}, {"KWSP_9C_AHL_D5.pdf"})
    ...
    def test_weak_unrelated_forms_are_removed_after_reranking(self):
        chunks = [
            {"source": "KWSP_9C_AHL_D5.pdf", "agency": "KWSP", "text": "house purchase", "page": 3},
            {"source": "KWSP_9C_AHL_D5.pdf", "agency": "KWSP", "text": "buy a house", "page": 4},
            {"source": "KWSP_Form_15.pdf", "agency": "KWSP", "text": "contribution withdrawal", "page": 1},
            {"source": "KWSP_9FA_AHL.pdf", "agency": "KWSP", "text": "Akaun Fleksibel", "page": 3},
        ]
    ...
    def test_topic_drift_is_removed_relative_to_strong_top_match(self):
        chunks = [
            {"source": "KWSP_9FA_(AHL)_BORANG_PERMOHONAN_PENGELUARAN_AKAUN_FLEKSIBEL.pdf", "text": "Akaun Fleksibel", "page": 1},
            {"source": "KWSP_9FA_(AHL)_BORANG_PERMOHONAN_PENGELUARAN_AKAUN_FLEKSIBEL.pdf", "text": "flexible withdrawal", "page": 2},
            {"source": "KWSP_Form_Pindah_Akaun_2_Ke_Akaun_1.pdf", "text": "account transfer", "page": 1},
        ]
        fused = [{"chunk": chunk, "score": 0.03} for chunk in chunks]
        selected = select_reranked_chunks(fused, [0.97, 0.53, 0.43])
        self.assertEqual({chunk["source"] for chunk in selected}, {
            "KWSP_9FA_(AHL)_BORANG_PERMOHONAN_PENGELUARAN_AKAUN_FLEKSIBEL.pdf"
        })

    def test_citations_are_derived_only_from_final_context(self):
        chunks = [
            {"source": "KWSP_9C_AHL_D5.pdf", "page": 3, "effective_date": None},
            {"source": "KWSP_9C_AHL_D5.pdf", "page": 4, "effective_date": None},
        ]
...
class ApiContractTests(unittest.TestCase):
    ...
    def test_attachment_context_reaches_answer_without_form_filling(self):
        ...
        official = {"id": "1", "source": "KWSP_9F_AHL_Borang_Pengeluaran.pdf", "agency": "KWSP", "text": "D3 requested withdrawal amount", "page": 2}
    ...
    def test_attachment_grounding_excludes_a_different_form_code(self):
        ...
        chunks = [
            {"source": "KWSP_Form_KWSP_R1_AHL.pdf", "agency": "KWSP", "text": "KWSP R1 (AHL)", "page": 1},
            {"source": "KWSP_Form_KWSP_9KM_AHL_BM.pdf", "agency": "KWSP", "text": "KWSP 9KM (AHL)", "page": 3},
        ]
        selected = _attachment_grounding_chunks(chunks, [record])
        self.assertEqual([chunk["source"] for chunk in selected], ["KWSP_Form_KWSP_R1_AHL.pdf"])
