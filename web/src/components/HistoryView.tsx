...
export function HistoryView({ chats, ready, disabled, onOpen, onClear }: { chats: ChatSession[]; ready: boolean; disabled: boolean; onOpen: (chat: ChatSession) => void; onClear: () => void }) {
  return (
    <section id="history" aria-labelledby="history-title" className="flex h-[610px] min-h-[560px] flex-col overflow-hidden rounded-2xl border border-[#d6e0e9] bg-white lg:h-full lg:min-h-0">
      <div className="flex items-center justify-between border-b border-[#e5eaf0] px-5 py-4">
        ...
        {chats.length > 0 && <button type="button" disabled={disabled} onClick={onClear} className="inline-flex min-h-9 items-center gap-1.5 rounded-lg px-2.5 text-[11px] font-bold text-[#7b3c48] hover:bg-[#fff2f4] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#a94b5c] disabled:opacity-50"><Trash2 className="size-3.5" aria-hidden="true" />Clear</button>}
      </div>
