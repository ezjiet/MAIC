...
export function ChatPanel({ chat, query, loading, interactionLocked, uploading, uploadError, pendingAttachments, activeAttachmentCount, needsReupload, errorKind, focusedMessageId, savedMessageIds, onQueryChange, onSubmit, onRetry, onToggleSave, onNewChat, onUpload, onRemoveAttachment }: ChatPanelProps) {
  const conversationRef = useRef<HTMLDivElement>(null);
  const fileRef = useRef<HTMLInputElement>(null);
  const busy = interactionLocked;
  ...
  return (
    <section aria-labelledby="chat-title" aria-busy={loading || uploading} className="flex h-[610px] min-h-[560px] flex-col overflow-hidden rounded-2xl border border-[#d6e0e9] bg-white shadow-[0_18px_38px_-34px_rgba(16,36,62,0.6)] lg:h-full lg:min-h-0">
      <div className="flex items-center justify-between gap-3 border-b border-[#e5eaf0] px-4 py-3 sm:px-5"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[0.14em] text-[#788697]">Clarify MY assistant</p><h2 id="chat-title" className="mt-0.5 truncate text-[15px] font-bold text-[#10243e]">{chat.title}</h2></div><button type="button" disabled={uploading} onClick={onNewChat} aria-label="Start a new chat" className="inline-flex min-h-8 shrink-0 items-center gap-1.5 rounded-lg border border-[#cfdbe6] bg-white px-2.5 text-[10px] font-extrabold text-[#315d85] transition hover:bg-[#f5f8fb] disabled:opacity-50"><Plus className="size-3.5" aria-hidden="true" />New Chat</button></div>
      ...
      <form onSubmit={submit} className="border-t border-[#e5eaf0] bg-white p-3 sm:px-4 sm:py-3">
        ...
        <div className="flex items-end gap-2 rounded-xl border border-[#cfd9e4] bg-white p-1.5 focus-within:border-[#2b65a5] focus-within:ring-3 focus-within:ring-[#2b65a5]/10">
          ...
          <button type="button" onClick={() => fileRef.current?.click()} disabled={busy || activeAttachmentCount >= 3} aria-label="Attach a PDF, JPG or PNG form" title="Attach form (PDF, JPG or PNG; max 10 MB)" className="grid size-10 shrink-0 place-items-center rounded-lg text-[#557087] hover:bg-[#edf3f7] disabled:opacity-40"><Paperclip className="size-4" aria-hidden="true" /></button>
          <label htmlFor="question" className="sr-only">Ask about KWSP, LHDN or JPJ</label><textarea id="question" rows={1} maxLength={500} value={query} disabled={busy} onChange={(event) => onQueryChange(event.target.value)} onKeyDown={keyDown} placeholder={pendingAttachments.length ? "Ask what a field means..." : "Ask about KWSP, LHDN or JPJ..."} className="min-h-10 max-h-24 flex-1 resize-none bg-transparent px-1 py-2.5 text-sm leading-5 text-[#10243e] outline-none placeholder:text-[#8b97a5]" />
